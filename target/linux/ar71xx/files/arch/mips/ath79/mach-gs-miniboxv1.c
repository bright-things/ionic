...
static const char *gs_miniboxv1_part_probes[] = {
	"tp-link",
	NULL,
};

static struct flash_platform_data gs_miniboxv1_flash_data = {
	.part_probes	= gs_miniboxv1_part_probes,
};
...
static struct gpio_keys_button gs_miniboxv1_gpio_keys[] __initdata = {
	{
		.desc		= "reset",
		.type		= EV_KEY,
		.code		= KEY_RESTART,
...
static void __init gs_miniboxv1_setup(void)
{
	u8 *mac = (u8 *) KSEG1ADDR(0x1f01fc00);
	u8 *ee = (u8 *) KSEG1ADDR(0x1fff1000);

	ath79_register_leds_gpio(-1, ARRAY_SIZE(gs_miniboxv1_leds_gpio),
				 gs_miniboxv1_leds_gpio);

	ath79_register_gpio_keys_polled(-1, GS_MINIBOXV1_KEYS_POLL_INTERVAL,
					ARRAY_SIZE(gs_miniboxv1_gpio_keys),
					gs_miniboxv1_gpio_keys);

	ath79_register_usb();

	ath79_register_m25p80(&gs_miniboxv1_flash_data);
